...
```bash
git add src/theme/tokens.css src/App.tsx src/components/Nav.tsx src/components/ActiveWorkoutBar.tsx src/components/RestBar.tsx src/components/RestWatcher.tsx e2e/core.spec.ts
...
Login uses one strong Overload wordmark, short value copy, one primary Google action, and a quiet recovery action. Home uses one accent next-action panel, a plain week metric band/consistency strip, and grouped recent activity. Train separates program labels from routine rows and uses action buttons only where an action exists. History uses month headings plus grouped rows, not a full page of identical cards. Navigation labels remain 12px minimum and active state has no decorative dot.
...
Run: `pnpm i18n && pnpm e2e --project=chromium && pnpm build`
...
Use PageHeader, a stable media ratio, title/alternate name, plain metadata, latest performance, Technique panel, instructions, deferred video, and Journal. Stop auto-running an infinite crossfade when reduced motion is requested; use a static first frame.
...
```js
const flatValues = (obj) => Object.values(obj).flatMap((value) =>
  typeof value === 'object' && value !== null ? flatValues(value) : [String(value)],
);
const forbidden = /\b(bulk|massa|surplus|whey)\b/i;
for (const [locale, data] of [['it', load('it.json')], ['en', load('en.json')]]) {
...
Remove meal example state/UI/copy and numeric target placeholders. Nutrition inputs use persistent labels and optional user targets. Body copy describes weekly average as a trend, not a bulk tool, and weight follows the selected unit. Training progress adapts chart/labels to weight, repetitions, or duration data. Replace three cramped metric cards with a plain aligned metric band and keep one main chart.

- [ ] **Step 4: Verify this screen group**

Run: `pnpm i18n && pnpm test && pnpm e2e --project=chromium && pnpm build`
...
test('profile switches units and import export exposes complete backup', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /complete backup|backup completo/i })).toBeVisible();
...
Remove program start. Add one kg/lb segmented control with full labels. Keep sync state in text, not color alone. Make each settings row a native control with clear label/value hierarchy. Render `ExportRows` inside ImportExport's export group, show version-2 collection counts in preview, disable import while busy with a visible progress label, and retain valid error toast.
...
```bash
git add src/screens/Profile.tsx src/screens/ImportExport.tsx src/theme/tokens.css src/i18n/it.json src/i18n/en.json e2e/core.spec.ts
...
- Modify: every screen that needs catalog data
...
Remove eager `loadCatalog` from store `init`. `ensureCatalog` returns immediately when ready, otherwise shares the existing catalog promise, registers custom exercises after resolve, and sets `catalogReady`. Home requests the catalog only when it has workouts to name; Train does not need it until Routine Editor or Workout opens. Schedule an idle prefetch after Home's useful paint using `requestIdleCallback` with a 2s timeout fallback, canceling on unmount.

- [ ] **Step 3: Lazy-load route modules**

Keep Home eager. Lazy-load Train, Profile, Workout, Summary, WorkoutDetail, Progress, Library, ExerciseSheet, ImportExport, RoutineEditor, and History. Use one minimal skeleton matching page header and two content rows; preserve active-route recovery under Suspense.
...
```bash
git add src/App.tsx src/state/useStore.ts src/hooks/useCatalog.ts src/screens vite.config.ts
...
Run: `rg -n "—|–|Operazione Rientro|bulk|massa|surplus|whey|tell me|dimmelo|programStart|phase1|deload" src README.md PRODUCT.md`

Expected: no visible-product matches; hyphens inside identifiers are not matched by this expression.
...
### Task 9: Fresh external UI/a11y review and production deployment
